...
int main()
{
	...
	while (!glfwWindowShouldClose(window))
	{
		...
		case PBR:
			...
			break;
		case Cellshade:
			m_RenderEngine->RenderCellShaded();
			break;
		}
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods)
{
	if (glfwGetKey(window, GLFW_KEY_T) == GLFW_PRESS)
	{
		...
		{
			m_ShadingState = static_cast<ShaderType>(static_cast<int>(ShaderType::Start) + 1);;
		}
	}
	...
	{
		m_ShadingState = static_cast<ShaderType>(static_cast<int>(m_ShadingState) - 1);
		if (m_ShadingState == ShaderType::Start)
		{
			m_ShadingState = static_cast<ShaderType>(static_cast<int>(ShaderType::END) - 1);
		}
	}
}